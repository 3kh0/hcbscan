...
<template>
  <div class="space-y-6">
    ...
    <template v-else-if="data">
      ...
      <UCard v-if="data.activityVolume.length" padding="p-6">
        ...
        <div class="flex items-end gap-0.5 h-40">
          ...
          >
            ...
            ></div>
            <div
              class="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-zinc-700 text-text-primary text-xs px-2 py-1 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 group-hover:translate-y-0 translate-y-1 scale-95 group-hover:scale-100 pointer-events-none transition-all duration-150 z-10"
            >
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-zinc-700">
              <tr v-for="o in data.topOrgs" :key="o.id" class="text-sm">
                <td class="py-3 text-text-muted">{{ o.rank }}</td>
                <td class="py-3">
                  <NuxtLink
                    :to="`/app/org/${o.id}`"
            ...
            <table class="w-full">
              ...
              <tbody class="divide-y divide-zinc-700">
                <tr
                  ...
                  class="text-sm h-13"
                >
                  <td class="py-3 text-text-muted">{{ u.rank }}</td>
                  <td class="py-3">
                    <NuxtLink
                      :to="`/app/usr/${u.id}`"
                      class="text-blue-400 hover:underline"
