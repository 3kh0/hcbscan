<script setup lang="ts">
  ...
  const pageTitle = computed(() =>
    orgMeta.value?.name
      ? `${orgMeta.value.name} Invoices - HCBScan`
      : "Organization Invoices - HCBScan"
  );
  const pageDescription = computed(() =>
    orgMeta.value?.name
      ? `All invoices for ${orgMeta.value.name} on HCBScan`
      : "View all invoices for an organization on HCBScan"
  );
